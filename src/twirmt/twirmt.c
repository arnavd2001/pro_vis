...
static char Rcs_Id[] ATTRIBUTE((used)) =
    "$Id: twirmt.c,v 1.28 2014-01-23 21:11:42-08 geoff Exp $";
#endif
 ...
 * rid of leftover optimizer-bug stuff.  Stop using twirmt_guts.h;
...
int mts_savestate(
	...
	{
	fprintf(stderr,
	  "twirmt internal: Trying to write invalid state pointer %d\n",
	  state->stateptr);
	mts_refresh(state);
	}
